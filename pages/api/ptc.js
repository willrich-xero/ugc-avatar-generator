// pages/api/ptc.js
// Starts a Flora piece-to-camera-generator run.
// Takes same 4 image inputs as angle generator:
// [fullBodyFront, fullBodySide, expressions, environmentShot]
// Returns runId immediately — browser polls /api/poll for results.
// Outputs 12 images: 4 office, 4 car, 4 park.
...
export default async function handler(req, res) {
  ...
  if (!images || images.length !== 4) {
    return res.status(400).json({ error: '4 images are required: fullBodyFront, fullBodySide, expressions, environmentShot' })
  }

  const apiKey = process.env.FLORA_API_KEY
  const techniqueSlug = 'piece-to-camera-generator'
  ...
  try {
    ...
    console.log('PTC technique inputs:', JSON.stringify(technique.inputs?.map(i => ({ id: i.id, type: i.type })), null, 2))

    const imageInputs = technique.inputs?.filter(i => i.type === 'imageUrl' || i.type === 'image') ?? []

    if (imageInputs.length < 4) {
      return res.status(500).json({
        error: `Expected at least 4 image inputs, found ${imageInputs.length}`,
        inputs: technique.inputs,
      })
    }

    // Step 2: build inputs array
    ...
    console.log('Inputs:', JSON.stringify(inputs.map(i => ({ id: i.id, type: i.type, value: i.value.substring(0, 80) })), null, 2))
    ...
    return res.status(500).json({
      error: err.message || 'Failed to start PTC generator run',
      name: err.name,
