...
export default function PTCGenerator() {
  ...
  async function saveShots() {
    if (!selectedShots.length || !selectedAvatarId) return
    setSaving(true)
    ...
    const newPTCShots = selectedShots.map(i => ({
      url: outputs[i]?.url,
      label: `PTC Shot ${i + 1}`,
  ...
  async function startGeneration() {
    ...
    setSelectedShots([])
    setSaved(false)
    setProgressLabel('Starting PTC generation...')
  ...
  async function pollStatus(id, slug) {
    try {
      ...
      if (data.status === 'running' || data.status === 'pending') {
        setProgressLabel(`Generating PTC shots... ${data.progress ?? 0}%`)
      }
      if (data.status === 'completed') {
        clearInterval(pollRef.current)
        setGenStatus('done')
        setProgress(100)
        setProgressLabel('All PTC shots generated')
      ...
      <div style={{ maxWidth: 720, margin: '0 auto', padding: '40px 24px', fontFamily: 'system-ui, -apple-system, sans-serif', color: '#1A2B4A' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 32 }}>
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 4 }}>
              ...
              <h1 style={{ fontSize: 22, fontWeight: 600, margin: 0 }}>PTC generator</h1>
            </div>
            <p style={{ fontSize: 14, color: '#4A5568', paddingLeft: 20, margin: 0 }}>Generate piece-to-camera shots — office, car and park</p>
          </div>
        ...
        {step === 2 && (
          <div>
            ...
            <p style={{ fontSize: 13, color: '#4A5568', marginBottom: 14 }}>The office environment is used as the character reference for generating all three locations.</p>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              ...
              <Btn primary disabled={!selectedEnvUrl} onClick={() => { setStep(3); setGenStatus('idle') }}>Generate PTC shots →</Btn>
            </div>
          </div>
        )}
        ...
        {step === 3 && (
          <div>
            {genStatus === 'idle' && (
              <Notice>Ready to generate 12 PTC shots — 4 office, 4 car, 4 park. This may take a few minutes.</Notice>
            )}
            ...
            {/* Output grid — grouped by location */}
            {outputs.length > 0 && (
              <div>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 8, marginBottom: 4 }}>
                  <span style={S.sectionLabel}>Generated shots</span>
                  <span style={{ fontSize: 12, color: '#4A5568' }}>Click to select, then save to library</span>
                </div>
                        ...
                        <div style={{ padding: '8px 10px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                          ...
                          {output?.url && (
                            <div style={{ display: 'flex', gap: 8 }}>
                              ...
                              <a href={output.url} download={`ptc-shot-${i + 1}.jpg`} onClick={e => e.stopPropagation()} style={{ fontSize: 12, color: '#13B5EA', textDecoration: 'none' }}>↓</a>
                            </div>
                          )}
                        </div>
                      </div>
                ...
                {saved ? (
                  <div style={{ padding: '12px 16px', background: '#EAF7EF', borderRadius: 8, border: '1px solid #27AE60', fontSize: 13, color: '#27AE60', fontWeight: 500, marginBottom: 12 }}>
                    ✓ {selectedShots.length} PTC shot{selectedShots.length !== 1 ? 's' : ''} saved to {selectedAvatar?.name}'s library
                  </div>
                ) : (
